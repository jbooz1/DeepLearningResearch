...
def main():
    ...
    perms_acc = perms_df.get("avg_acc").values

    #features x and y
    features_ratios = features_df.index.tolist()
    features_acc = features_df.get("avg_acc").values

    #combined x and y
    combined_ratios = combined_df.index.tolist()
    combined_acc = combined_df.get("avg_acc").values
    ...
    #plot formatting
    plt.title('Accuracy vs. input data source', fontsize=18)
